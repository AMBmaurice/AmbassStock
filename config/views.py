...
def page_inventaire(request):
    ...
    # **3. GESTION DU CRÉNEAU DE BLOCAGE DES COMMANDES (Mercredi 12h00 -> Jeudi 17h00)**
    maintenant = timezone.now()
    jour_semaine = maintenant.weekday()  # 0=Lundi, 2=Mercredi, 3=Jeudi...
    heure_actuelle = maintenant.time()
    ...
    if request.method == "POST":
        ...
        # **4. SÉCURITÉ : EMPÊCHER L'AJOUT AU PANIER SI BLOQUÉ (SAUF POUR L'ADMIN)**
        if action_type in ["ajouter_panier_groupe", "ajouter_panier"] and panier_bloque and not is_admin:
            messages.error(
                request, 
                "Les ajouts au panier sont fermés du mercredi 12h00 au jeudi 17h00 afin de préparer les retraits. Aucune modification n'est acceptée durant ce créneau."
        ...
        # **5. SOUMISSION GROUPÉE DE LA LISTE DE COURSES (PANIER FLOTTANT)**
        if action_type == "ajouter_panier_groupe":
            service = request.POST.get('service')
            panier_raw = request.POST.get('panier_json')
        ...
        # GESTION AJOUT AU PANIER UNIQUE
        elif action_type == "ajouter_panier":
            produit_id = request.POST.get('produit_id')
            service = request.POST.get('service')
            quantite = int(request.POST.get('quantite', 1))
        ...
        # **6. MODIFICATION PRODUIT (ADMIN) AVEC SAUVEGARDE DU FOURNISSEUR**
        elif action_type == "modification" and is_admin:
            produit_id = request.POST.get('produit_id')
            current_page = request.POST.get('page', '1')
            recherche_term = request.POST.get('q', '')
        ...
        # SUPPRESSION DÉFINITIVE (SUPERUSER)
        elif action_type == "suppression_definitive" and request.user.is_superuser:
            produit_id = request.POST.get('produit_id')
            current_page = request.POST.get('page', '1')
            recherche_term = request.POST.get('q', '')
    ...
    # **7. RÉCUPÉRATION COMPLÈTE POUR LA RECHERCHE JS GLOBALE**
    tous_les_produits_complets = list(tous_les_produits)
    ...
    # **8. RENDU DU TEMPLATE AVEC TOUT LE CONTEXTE NÉCESSAIRE**
    return render(request, 'inventaire.html', {
        'profil_actif': profil_actif,
        'is_admin': is_admin,
        'panier_bloque': panier_bloque,
...
def page_panier(request):
    ...
    if request.method == "POST":
        ...
        # Valider la livraison d'un service complet (Déduction du stock physique)
        if action_type == "valider_livraison_service":
            if not is_admin:
                messages.error(request, "Seule l'administration est autorisée à valider les livraisons.")
            ...
            if articles_service.exists():
                for item in articles_service:
                    prod = item.produit
                    # Déduit la quantité en s'assurant que le stock ne tombe pas sous 0
                    prod.quantite = max(0, prod.quantite - item.quantite_demandee)
                    prod.save()
                
                # Suppression des articles du panier après livraison
                articles_service.delete()
                messages.success(request, f"La commande du service {service_nom} a été livrée et le stock a été mis à jour avec succès !")
            else:
                messages.warning(request, f"Aucune commande active trouvée pour le service {service_nom}.")

            return redirect('/panier/')

        # Retirer un article spécifique d'une commande
        elif action_type == "retirer_item":
            item_id = request.POST.get('item_id')
            if item_id:
                ArticlePanier.objects.filter(id=item_id).delete()
                messages.success(request, "L'article a été retiré de la liste.")
            return redirect('/panier/')
