...
def page_inventaire(request):
  ...
  if request.method == 'POST':
    ...
    # **1. CRÉATION D'UN NOUVEAU PRODUIT AVEC CONSERVATION DES CENTIMES**
    if action_type in ['ajout', 'ajouter_produit', 'creation'] and is_admin:
      try:
        ...
        # **TRAITEMENT ET ARRONDI DU PRIX À 2 DÉCIMALES (CONSERVE LES CENTIMES)**
        prix_raw = request.POST.get('prix')
        prix = None
        if prix_raw and str(prix_raw).strip():
          try:
            prix = round(float(str(prix_raw).replace(',', '.').strip()), 2)
          except ValueError:
            prix = None
        ...
        messages.success(
            request, f'Le produit "{objet}" a été créé avec succès !'
        )
      except Exception as e:
        messages.error(
            request, f'Erreur lors de la création du produit : {e}'
        )
    ...
    # **2. MODIFICATION D'UN PRODUIT AVEC CONSERVATION DES CENTIMES**
      ...
      try:
        ...
        quantite_raw = request.POST.get('quantite')
        produit.quantite = (
            int(quantite_raw) if quantite_raw and str(quantite_raw).strip() else 0
        )
...
def page_gestion_stocks(request):
    ...
    if request.method == 'POST':
        ...
        if action_type in ['creation', 'creation_produit']:
            ...
            if not objet_nom or not categorie_nom:
                messages.error(request, "Veuillez remplir au moins la désignation et la catégorie.")
            ...
            try:
                nouveau_produit = Produit.objects.create(
                    ...
                    emplacement=request.POST.get('emplacement') or 'Armoire A',
                    quantite=quantite_initiale,
                    quota_minimum=int(request.POST.get('quota_minimum', 5)),
                    fournisseur=fournisseur_final[:100],
                    prix=prix_valeur,
                ...
                messages.success(request, f'Le produit "{objet_nom}" ({reference_finale}) a été enregistré avec succès !')
            except Exception as e:
            ...
            try:
                ...
                produit.save(update_fields=['quantite', 'derniere_activite'])
                ...
                messages.success(request, f'Entrée de {quantite_ajoutee} unités enregistrée pour "{produit.objet}".')
            except Produit.DoesNotExist:
                messages.error(request, 'Produit introuvable en base de données.')
            except Exception as e:
            ...
            try:
                ...
                messages.success(request, f'Sortie de {quantite_retiree} x "{produit.objet}" enregistrée pour le service {service_demandeur}.')
            except Produit.DoesNotExist:
                messages.error(request, 'Produit introuvable en base de données.')
            except Exception as e:
            ...
            try:
                ...
                nom_p = produit.objet
                produit.delete()
                messages.success(request, f'Le produit "{nom_p}" a été supprimé.')
            except Produit.DoesNotExist:
